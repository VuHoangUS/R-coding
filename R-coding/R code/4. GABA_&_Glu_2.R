...
# Loading data 
GABAergic2 <- read.csv("https://raw.githubusercontent.com/VuHoangUS/R-coding/refs/heads/main/R-coding/Data/GABA2.csv")
...
NNAT_Glu <- Glutamatergic2[3, ]
NNAT_Non <- Nonneuronal2[3, ]


## STMN1
STMN1_GABA <- GABAergic2[4, ]
STMN1_Glu <- Glutamatergic2[4, ]
STMN1_Non <- Nonneuronal2[4, ]

## STMN2
STMN2_GABA <- GABAergic2[5, ]
STMN2_Glu <- Glutamatergic2[5, ]
STMN2_Non <- Nonneuronal2[5, ]

## BASP1
BASP1_GABA <- GABAergic2[1, ]
BASP1_Glu <- Glutamatergic2[1, ]
BASP1_Non <- Nonneuronal2[1, ]

## ITM2B
ITM2B_GABA <- GABAergic2[2, ]
ITM2B_Glu <- Glutamatergic2[2, ]
ITM2B_Non <- Nonneuronal2[2, ]
...
ggplot(NNAT, aes(x = symbol, y = Values, shape = symbol)) +
  ...
  labs(
    ...
    y = "log2 RPKM",
    subtitles = ifelse(res_NNAT$p.value < 0.0001, 
...
ggplot(STMN2, aes(x = symbol, y = Values, shape = symbol)) +
  ...
  labs(
    ...
    x = "Cell Type Broad Class",
    y = "log2 RPKM",
    subtitles = ifelse(res_STMN2$p.value < 0.0001, 
                       "Krusral - Wallis, p < 0.0001",
                       paste("Krusral - Wallis, p =", 
...
ggplot(BASP1, aes(x = symbol, y = Values, shape = symbol)) +
  ...
  labs(
    ...
    x = "Cell Type Broad Class",
    y = "log2 RPKM",
    subtitles = ifelse(res_BASP1$p.value < 0.0001, 
                       "Krusral - Wallis, p < 0.0001",
                       paste("Krusral - Wallis, p =", 
...
ggplot(ITM2B, aes(x = symbol, y = Values, shape = symbol)) +
  ...
  labs(
    ...
    x = "Cell Type Broad Class",
    y = "log2 RPKM",
    subtitles = ifelse(res_ITM2B$p.value < 0.0001, 
                       "Krusral - Wallis, p < 0.0001",
                       paste("Krusral - Wallis, p =", 
